...
public:
	CSingleList();
	~CSingleList();
	//插入：链表末尾插入， 成功返回非空指针，否则失败
	CElement* Insert(void* lpData, int iDataSize);
	//插入：链表指定位置插入， 成功范围非空指针，否则失败
	...
	//链尾
	CElement *End();
	//是否为空，空返回TRUE， 非空返回FALSE
	bool Empty();
	//反转
	bool Reverse();
	//检测环，存在环，返回TRUE， 否则返回FALSE
	bool CheckCircle();
	//合并两个有序链表
...
void PrintList(CSlingleList &lst)
{
	CElemeng* lpElement = lst.Begin();
