...
class EM:
    def __init__(self, prob):
        self.probA, self.probB, self.probC = prob

    # E step
    def pmf(self, i):
        prob1 = self.probA * math.pow(self.probB, data[i]) * math.pow(
            (1 - self.probB), 1 - data[i])
        prob2 = (1 - self.probA) * math.pow(self.probC, data[i]) * math.pow(
            (1 - self.probC), 1 - data[i])
        return prob1 / (prob1 + prob2)

    # M step
    def fit(self, data):
        count = len(data)
        print('init prob:{}, {}, {}'.format(self.probA, self.probB, self.probC))

        for d in range(count):
            _ = yield  # 函数执行到这会停顿
            _pmf = [self.pmf(k) for k in range(count)]
            probA = 1 / count * sum(_pmf)
            probB = sum([_pmf[k] * data[k] for k in range(count)]) / sum(
                [_pmf[k] for k in range(count)])
            probC = sum([(1 - _pmf[k]) * data[k]
                         for k in range(count)]) / sum([(1 - _pmf[k])
                                                        for k in range(count)])
            print('{}/{}  probA:{:.3f}, probB:{:.3f}, probC:{:.3f}'.format(
                d + 1, count, probA, probB, probC))
            self.probA = probA
            self.probB = probB
            self.probC = probC
